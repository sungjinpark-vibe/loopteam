...
| T012 | Art resource upgrade — Blender-modeled village landmark | `blocked` | ui-ux | build | 1 | 5-round limit hit at 44/95 — see task log. Root cause: `mcp__blender` grant didn't reach the subagent this session (needs restart). Score history + fix list logged; not lost. |
