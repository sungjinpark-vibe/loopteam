...
- **Persistence with zero extra writes:** `TownState` gains **optional** `moveHintSeen?: boolean`. It is set in memory when the hint is satisfied and **folded into the next `saveCore` the app performs for any other reason** (every entry save and every claim writes core already). The store never issues a write purely for the hint, so a move still writes exactly one key (§4.2) and a cold boot writes nothing. Worst case, a kill between "hint seen" and the next save shows the hint once more — acceptable, and cheaper than a dedicated write.
